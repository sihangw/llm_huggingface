...
def collect_and_save_dataset(
    ...
    return dataset, insts

def save_step_functions(step_fns: Dict[int, Any], save_path: str):
    with open(save_path, "wb") as f:
        dill.dump(step_fns, f)
    print(f"Saved {len(step_fns)} step functions to {save_path}")

def save_insts(insts, save_path: str):
    with open(save_path, "wb") as f:
        dill.dump(insts, f)
    print(f"Saved {len(insts)} instances to {save_path}")
...
if __name__ == "__main__":
    ...
    fns_path = os.path.join(out_dir, "step_functions.dill")
    inst_path = os.path.join(out_dir, "instances.dill")
    ...
    dataset, insts = collect_and_save_dataset(
        sampler=sampler,
        num_trials=10000,
        out_path=jsonl_path,
    ...
    print(paths)
    # save_step_functions(step_fns, save_path=fns_path)
    save_insts(insts, save_path=inst_path)